...
class AppTray {
  constructor(dialogWindow, mainWindow) {
  ...
  createTray() {
    ...
    if (process.platform === 'linux' || process.platform === 'win32') {
      ...
      this.trayIcon = image
      this.trayIconUnread = nativeImage.createFromPath(
        path.join(assetsPath, `tray_unread_${this.trayColor}.png`)
      )
    } else {
  ...
  hideDialogAndShowMain() {
    if (this.dialogWindow.isShown) return
